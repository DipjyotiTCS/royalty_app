...
def calculateSales():
  ...
  # Convert row to SalesData object
    ...
    #Perform calculation
    canada_old_amount = (sales_data.sales_canada * request_data.royalty_table_canada)/100
    canada_new_amount = (sales_data.sales_canada * request_data.royalty_latest_canada)/100
    canada_diff = 0 if canada_old_amount == canada_new_amount else (canada_new_amount - canada_old_amount)
    
    chapter_old_amount = (sales_data.sales_chapter * request_data.royalty_table_chapter)/100
    chapter_new_amount = (sales_data.sales_chapter * request_data.royalty_latest_chapter)/100
    chapter_diff = 0 if chapter_old_amount == chapter_new_amount else (chapter_new_amount - chapter_old_amount)
    
    us_old_amount = (sales_data.sales_us * request_data.royalty_table_us)/100
    us_new_amount = (sales_data.sales_us * request_data.royalty_latest_us)/100
    us_diff = 0 if us_old_amount == us_new_amount else (us_new_amount - us_old_amount)
    
    foreign_old_amount = (sales_data.sales_foreign * request_data.royalty_table_foreign)/100
    foreign_new_amount = (sales_data.sales_foreign * request_data.royalty_latest_foreign)/100
    foreign_diff = 0 if foreign_old_amount == foreign_new_amount else (foreign_new_amount - foreign_old_amount)
    
    
    highDiscout_old_amount = (sales_data.sales_high_discount * request_data.royalty_table_high_discount)/100
    highDiscout_new_amount = (sales_data.sales_high_discount * request_data.royalty_latest_high_discount)/100
    highDiscout_diff = 0 if highDiscout_old_amount == highDiscout_new_amount else (highDiscout_new_amount - highDiscout_old_amount)
    
    
    stateAdoption_old_amount = (sales_data.sales_state_adoption * request_data.royalty_table_state_adoption)/100
    stateAdoption_new_amount = (sales_data.sales_state_adoption * request_data.royalty_latest_state_adoption)/100
    ...
    subUs_old_amount = (sales_data.sales_sub_us * request_data.royalty_table_sub_us)/100
    subUs_new_amount = (sales_data.sales_sub_us * request_data.royalty_latest_sub_us)/100
    subUs_diff = 0 if subUs_old_amount == subUs_new_amount else (subUs_new_amount - subUs_old_amount)
    
    subForeign_old_amount = (sales_data.sales_sub_foreign * request_data.royalty_table_sub_foreign)/100
    subForeign_new_amount = (sales_data.sales_sub_foreign * request_data.royalty_latest_sub_foreign)/100
    subForeign_diff = 0 if subForeign_old_amount == subForeign_new_amount else (subForeign_new_amount - subForeign_old_amount)
    
    subTrial_old_amount = (sales_data.sales_sub_trial * request_data.royalty_table_sub_trial)/100
    subTrial_new_amount = (sales_data.sales_sub_trial * request_data.royalty_latest_sub_trial)/100
